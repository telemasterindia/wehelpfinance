...
function getUtm(): Record<string, string> {
  ...
  // also pull from sessionStorage (set on first landing page)
  try {
...
// ─── Partial lead (phone captured) → Web3Forms ───────────────────────────────
async function sendPartialLead(data: LeadData): Promise<void> {
  ...
  const body = {
    access_key: W3F_KEY,
    subject: `⚡ Partial Lead — Phone Captured (${CAT_LABEL[data.category ?? "debt-relief"] ?? data.category})`,
    from_name: "WeHelpFinance Partial Lead",
    // lead data
    phone: data.phone ?? "",
    category: CAT_LABEL[data.category ?? "debt-relief"] ?? data.category ?? "",
  ...
  try {
    ...
    // Silent fail — local save already done above
  }
}

// ─── Final lead submission → Web3Forms ───────────────────────────────────────
async function sendFinalLead(data: LeadData): Promise<{ ok: boolean; error?: string }> {
  ...
  // Build qualifier field based on category
  let qualifier = "";
  ...
  const body = {
    access_key: W3F_KEY,
    subject: `🎯 New Lead — ${CAT_LABEL[data.category ?? "debt-relief"] ?? data.category} | ${data.state ?? ""}`,
...
export function LeadForm({ defaultCategory }: { defaultCategory?: Category }) {
  ...
  // Phone blur → partial lead
  async function onPhoneBlur() {
    if (!partialSentRef.current && data.phone && data.phone.length >= 10) {
    ...
    <div className="surface-card">
      ...
      {step === 3 && (
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          <StepHeader
            title="Where should our specialist reach you?"
            onBack={() => setStep(2)}
          />
          ...
          <Field label="Phone number" hint="A specialist will call you back — usually within 1 business day.">
          ...
          <button
            ...
            className="btn-cta w-full disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {submitting ? (
