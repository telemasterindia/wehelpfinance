...
export const metadata: Metadata = {
  ...
  twitter: {
    ...
    title: "Research Center | WeHelpFinance",
    description: "Consumer debt research, credit card statistics, and financial hardship data.",
  },
};
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-primary-soft/40 to-background">
        <div className="container-page max-w-4xl py-14 lg:py-20">
          ...
          <p className="mt-4 max-w-2xl text-lg text-muted-foreground">
            Data-driven research on consumer debt, credit card trends, inflation impact, and financial hardship facing American households.
            Published for consumers, journalists, and policymakers.
          </p>
      ...
      <div className="container-page max-w-5xl py-12 pb-24">
        ...
        <div className="mb-10 flex flex-wrap gap-2">
          <span className="rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">All Research</span>
          {CATEGORIES.map((c) => (
            <span key={c} className="rounded-full border border-border bg-card px-4 py-2 text-sm font-medium text-muted-foreground hover:border-primary hover:text-primary cursor-pointer transition-colors">
              {CATEGORY_LABELS[c]}
            </span>
          ))}
        </div>
        ...
        <section className="mt-14 rounded-3xl border border-border bg-card p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href="/about" className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline">
              About WeHelpFinance <ArrowRight className="h-4 w-4" aria-hidden="true" />
            </Link>
            <Link href="/editorial-policy" className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline">
              Editorial Policy <ArrowRight className="h-4 w-4" aria-hidden="true" />
            </Link>
          </div>
        </section>
      </div>
...
function ReportCard({
  ...
  return (
    ...
    >
      ...
      <h3 className={`font-display font-semibold text-foreground group-hover:text-primary leading-snug ${featured ? "text-lg" : "text-base"}`}>
        {report.title}
      </h3>
      ...
      {report.keyFindings.length > 0 && (
        <div className="mt-4 rounded-xl bg-muted/50 p-3">
          <p className="text-xs font-semibold text-foreground mb-1.5">Key finding:</p>
          <p className="text-xs text-muted-foreground leading-relaxed">{report.keyFindings[0]}</p>
        </div>
      )}
      ...
      <div className="mt-5 flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span className="flex items-center gap-1 font-medium text-primary">
          Read report <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" aria-hidden="true" />
        </span>
      </div>
    </Link>
  );
